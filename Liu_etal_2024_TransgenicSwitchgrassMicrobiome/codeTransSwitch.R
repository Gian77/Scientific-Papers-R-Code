...
# NOTES
# Control samples headed no reads so are not present in the data sets.
# Figure1 - alpha div. Observed and inverse Simpson. Use barplot for just
#           Pre-Transplant and Post-Transplant.
# Figure2 - Main ordination with all samples and treatments together.
# Figure3 - Ordinations spitted by Niche. 
# Figure4 - Differential abundance. 
# Figure5 - maybe a network?
#
# Table1 - Permanova on the whole dataset.
# Table2 - Permanova on datasets split by Niche.
...
mutate_if(is.numeric, funs(x=./sum(x=.)))
mutate(across(where(is.numeric),~rowSums(x = .)))
mutate(Sum = rowSums(x = .)) %>% 
  mutate(Abund = value / sum(value)) 
...
StackedBarsDF(fungi_ev_100itr, fungi_taxonomy_fix, meta_fungi_100itr, Phylum) %>% 
  mutate(Group = paste(Niche, Status, Treatment, sep = "_")) %>% 
  dplyr::select(Phylum, value, Group) %>% 
  group_by(Group, Phylum) %>% 
  summarise(Mean = mean(value)) %>% 
  ungroup() %>% 
  nest(data=-Group) %>% 
  mutate(Abund = map(.x=data, ~relabund(.x$Mean), data = .x)) %>%
  unnest(c(Group, data, Abund)) %>% 
  separate(Group, c("Niche", "Status","Treatment"), sep = "_") %>% 
  mutate(Niche = as.factor(Niche), 
         Niche = fct_relevel(Niche, "Inflorescence","Leaf", "Root", 
                                    "Rhizosphere","Soil")) %>% 
  mutate(Status = as.factor(Status), 
         Status=fct_relevel(Status, "Pre-Transplant",
                                    "Post-Transplant")) %>% 
  ...
  dplyr::select(-Mean) %>% 
  pivot_wider(id_cols = Phylum, names_from = c(Niche, Status, Treatment), 
              values_from = Abund) %>% as.data.frame()
    
  
  
  
  ggplot(aes(x=Treatment, y=Abund, fill=Phylum, color=Phylum)) +
    geom_bar(stat = "identity") +
    facet_wrap(Status~Niche, ncol = 8, nrow = 1) +
  theme_bw() +
  theme(strip.text.x = element_text(angle = 0, size = 9,hjust = 0.5, vjust = 0.5, color = "black"),
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5,color = "black"),
        legend.key.height = unit(0.4, "cm"), legend.key.width = unit(0.4, "cm"),
        ...
        strip.background = element_blank(), 
        legend.position = "bottom") +
  scale_fill_manual(values = paletteCB30) +
  scale_color_manual(values = paletteCB30) 
...
df %>%
  dplyr::select(Taxonomy, value, Treatment) %>% 
  mutate(Label = paste(Taxonomy,Treatment, sep = "=")) %>%
  group_by(Label) %>% 
  summarise(Mean = mean(value),
            SD = sd(value)) %>% 
  separate(Label, c("Taxonomy", "Treatment"), sep = "=") %>% 
  ggplot(aes(x = Mean, y = Taxonomy, fill = Treatment), 
         colour="black") +
  geom_bar(stat = "identity", position = position_dodge(), color="black") +
  geom_errorbar(aes(xmin = Mean, xmax = Mean + SD), color="black",
                width = 0.2,
...
# Calculating pairwise wilcox.test across variables for each Genus
GetWilcoxLetters <- function(df, Var, formula){
  require(tidyverse)
  require(broom)
  Var <- enquo(Var) 
  wilk_test <- 
    df %>%
    dplyr::select(Taxonomy, Abund, !!Var) %>% 
    nest(data=-Taxonomy) %>% 
    mutate(Sig = purrr::map(.x=data, ~CompSampl(df = ., formula), data = .x),
           Group = purrr::map(.x=Sig, ~rownames(x = .), data=.x)) %>%
    unnest(c(Sig, Group)) %>% 
    dplyr::select(Taxonomy, Group, Letters) %>% 
    pivot_wider(names_from = Group, values_from = Letters)
  return(wilk_test)
}
#mutate(Log_Abund = log(Abund + 1/20000)) %>% 
#mutate(Var = fct_relevel(!!Var, "Leaf","Nearsoil", "Farsoil")) %>%
#mutate(Taxonomy = ifelse(is.na(Taxonomy), paste("Unclassified"),paste(Taxonomy))) %>% 
# WIlcox test across groups --------------------------------------------
# Getting wilcox test across Slice and removing non-significant comparisons
split_fungi_pre_soil_letters <-
  split_fungi_pre_soil_tax %>% 
  mutate(Treatment = as.factor(Treatment)) %>% 
  GetWilcox(df = ., Treatment, formula(Abund ~ Treatment)) %>% 
filter(!if_all(GMO:`non-GMO`, ~ `non-GMO` == .x)) 
...
df_abund_fungi_otu <-
  ...
  ungroup() %>%
  pivot_longer(cols=c(-Taxonomy),names_to="SampleID") %>% 
  full_join(
    x = .,
...
# Calculating pairwise wilcox.test across variables for each Genus
GetWilcox <- function(df, Var, formula){
  Var <- enquo(Var) 
  wilk_test <- df %>%
    mutate(Log_Abund = log(Abund + 1/20000)) %>% # not used
    select(Taxonomy, Abund, !!Var) %>% 
    #mutate(Var = fct_relevel(!!Var, "Leaf","Nearsoil", "Farsoil")) %>%
    #mutate(Taxonomy = ifelse(is.na(Taxonomy), paste("Unclassified"),paste(Taxonomy))) %>% 
    nest(data=-Taxonomy) %>% 
...
otu_letters_fungi_slice <-
  GetWilcox(df_abund_fungi_otu, Slice, formula((Abund ~ Slice))) %>% 
  filter(!if_all(Control:Nearsoil, ~ Nearsoil == .x)) 


# Generating relative abundances in each variables ---------------------------------
df_barplot_fungi %>%
  select(Class, Abund, Slice) %>% 
  mutate(Slice = fct_relevel(Slice, "Leaf","Nearsoil", "Farsoil")) %>%
  mutate(Class = ifelse(is.na(Class),paste("Unclassified"),paste(Class))) %>% 
  nest(-Class, -Slice) %>% 
  mutate(Abund = map(.x=data, ~sum(x=.), data = .x)) %>%
  unnest(Abund) %>% 
  select(Class, Slice, Abund) %>% 
  pivot_wider(names_from = Slice, values_from = Abund)



Line 5009 script: new_script_microcosm_19

> df_barplot_fungi
# A tibble: 2,772 × 10
# Groups:   Class [11]
Class          SampleID value Slice    Treatment Pore  Plant Moisture Time     Abund
<chr>          <chr>    <dbl> <fct>    <fct>     <fct> <fct> <fct>    <fct>    <dbl>
  1 Agaricomycetes R1_49_N      0 Nearsoil T1        Large Corn  High     7     0       
2 Agaricomycetes R1_49_P      0 Leaf     T1        Large Corn  High     7     0       
3 Agaricomycetes R1_50_F      2 Farsoil  T1        Large Soy   High     7     0.000576
4 Agaricomycetes R1_50_N      7 Nearsoil T1        Large Soy   High     7     0.00201 
5 Agaricomycetes R1_50_P      0 Leaf     T1        Large Soy   High     7     0       
6 Agaricomycetes R1_51_N      2 Nearsoil T1        Large Corn  Low      7     0.000576
7 Agaricomycetes R1_51_P      1 Leaf     T1        Large Corn  Low      7     0.000288
8 Agaricomycetes R1_52_N      0 Nearsoil T1        Large Soy   Low      7     0       


# Calculating abundances across samples ------------------------------------------
# https://riffomonas.org/code_club/2021-07-02-wilcox.test

PlotJitt <- function(df, Var){
  Var <- enquo(Var)
  jitt_plot <- df %>%
    mutate(Log_Abund = -log(Abund + 1/20000),
           Class = ifelse(is.na(Class),paste("Unclassified"),paste(Class))) %>% 
    ggplot(aes(x=Abund, y=Class, color=!!Var, fill=!!Var)) +
    geom_jitter(position = position_jitterdodge(dodge.width = 0.8, jitter.width = 0.5), 
                shape=1) +
    stat_summary(aes(x=Abund), fun.data = mean_sdl, fun.args = list(mult=1), 
                 geom="pointrange",
                 position = position_dodge(width=0.8),
                 color="black", shape=5, size=0.15, #shape=16, size=0.5,
                 show.legend = FALSE) +
    scale_x_log10() +
    theme_classic() +
    #geom_vline(xintercept = 100/10530, size=0.5, color="gray") +
...
PlotJitt(df_barplot_fungi, Pore) +
  scale_color_manual(values=paletteCB5)
...
7 Agaricomycetes R1_51_P      1 Leaf     T1        Large Corn  Low      7     0.000288
8 Agaricomycetes R1_52_N      0 Nearsoil T1        Large Soy   Low      7     0       
9 Agaricomycetes R1_52_P      0 Leaf     T1        Large Soy   Low      7     0       
...
# ***********************************************--------------------------------
# Function to plot bars with significance values ------------------------------- 
PlotBarrs <- function(df, Var, formula, Par1, Par2, Par3=NULL){
  Var <- enquo(Var) 
  # First, calculate the pairwise wilcox.test.
  letters <-
    df %>% 
    GetWilcox(df = ., !!Var, formula) %>% 
    mutate(Class = as.factor(Class)) %>% 
    pivot_longer(!Class, names_to = "Group", values_to = "Sig") %>%
    mutate(Group = Group %>% 
             fct_relevel(Par1, Par2, Par3)) %>%
    arrange(.data = ., Class, Group) 
  as.data.frame(letters) %T>% print() 
  letters <- letters %>%  pull(3)
  print(letters)
  # Then, generate the plot.
  plot_bars<-
    df %>%
    mutate(Rel_Abund = 100 * (Abund + 1/20000),
           Class = ifelse(is.na(Class),paste("Unclassified"),paste(Class))) %>% 
    ggplot(aes(x=Rel_Abund, y=Class, fill = !!Var, color = !!Var)) +  
    #geom_bar(stat = "identity", position = position_dodge()) +
    geom_jitter(aes(x = Rel_Abund), position = position_jitterdodge(dodge.width = 0.8, jitter.width = 0.8), 
                size = 0.85, shape=1) +
    stat_summary(geom = "pointrange", show.legend = FALSE,
                 fun = mean, shape=5, size = 0.5, 
                 position = position_dodge(0.9), color="black") +
    stat_summary(geom = "errorbar", show.legend = FALSE,
                 fun.data = mean_sd, 
                 position = position_dodge(0.9), width = 0.3, color="black") +
    stat_summary(geom = 'text', label = letters, 
                 fun= mean, size=3, color="blue", 
                 position = position_dodge(0.9),
                 vjust = -0.2, hjust = -1.6) +
    theme_bw() +
    theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.5),
...
# Fungi 
df_barplot_fungi %>% 
  PlotBarrs(df = ., Slice, formula(Abund ~ Slice), "Leaf", "Nearsoil", "Farsoil") +
  scale_color_manual(values=paletteCB3) +
  scale_fill_manual(values=paletteCB3) +
  labs(title = "Slice", x= "Relative abundance %", y = NULL)
...
# wilcox.test
df_abund_fungi_genus %>%
  select(Genus, value, Slice) %>% 
  mutate(Slice = fct_relevel(Slice, "Leaf","Nearsoil", "Farsoil", "Control")) %>%
  mutate(Genus = ifelse(is.na(Genus), paste("Unclassified"), paste(Genus))) %>% 
  nest(data = -Genus) %>% 
  mutate(Sig = map(.x=data, ~CompSampl(df = ., formula(value ~ Slice)), data = .x)) %>% 
  mutate(Group = map(.x=Sig, ~rownames(x = .), data=.x)) %>%
  unnest(c(Sig, Group)) %>% 
  select(Genus, Group, Letters) %>% 
  pivot_wider(names_from = Group, values_from = Letters)
...
# 1) subsetting the dataframe to the desired samples and then rarefying the 
# distance matrix with avgdist()
meta_fungi_fix$Niche
...
dist_tibble <-
  ...
  left_join(x=., 
            y=meta_fungi_fix %>% 
              dplyr::select(Niche, Sample_ID), 
            by = "Sample_ID") %>% 
  filter(Niche%in%c("Root")) %>% 
  dplyr::select(-Niche) %>% 
  mutate(Sample_ID = gsub("ITS", "", Sample_ID)) %>% 
  pivot_wider(names_from ="ASV_ID", values_from = "Seq_No", values_fill = 0) %>% 
  column_to_rownames("Sample_ID") %>% 
...
set.seed(61)
nmds_fungi_soil <- metaMDS(dist_tibble, trymax = 200, 
                           autotransform = FALSE, engine = c("monoMDS"))
p1 <-
  PlotSplitNMDS(nmds_fungi_soil, meta_fungi_fix) +
  labs(title = "Fungal microbiome\nvegan avgdist on filtered data")
p1


# 2) Rarefying the whole dataset and then subsetting to the desired
# samples. Using rrarefy will generate 1 random subsampling without replacement.

dist_tibble2 <-
  df_filt_fungi %>% 
  vegan::rrarefy(x=., sample = min_depth_fungi) %>% 
  as.data.frame() %>% 
  rownames_to_column("Sample_ID") %>% 
  left_join(x=., 
            y=meta_fungi_fix %>% 
  ...
  vegdist(x=., dmethod="bray", sample=min_depth_fungi)

dist_tibble2

set.seed(62)
nmds_fungi_soil2 <- metaMDS(dist_tibble2, trymax = 200, 
                            autotransform = FALSE, engine = c("monoMDS"))
p2 <-
  PlotSplitNMDS(nmds_fungi_soil2, meta_fungi_fix) +
  labs(title = "Fungal microbiome\n vegan rrarefy - no iteration")

p2



# 3) Rarefying the whole dataset, and then subsetting to the desired
# samples, using phyloseq rarefy_even_depth.
...
# rarefying
set.seed(62)
sort(colSums(otu_table(physeq)), decreasing = TRUE)
physeq_ev = rarefy_even_depth(physeq, sample.size = min_depth_fungi, 
                              rngseed = FALSE, replace = TRUE, 
                              trimOTUs = TRUE, verbose = TRUE) 
otu_table(physeq_ev) <- 
  otu_table(physeq_ev)[which(rowSums(otu_table(physeq_ev)) >= 0),]

# filtering
physeq_ev <-
  subset_samples(physeq_ev, Niche%in%c("Root"))
otu_table(physeq_ev) <- 
  otu_table(physeq_ev)[which(rowSums(otu_table(physeq_ev)) > 0),] 

# pulling data out of phyloseq
physeq_df <-
  otu_table(physeq_ev) %>% 
  as.data.frame() %>% 
  t() %>% 
...
nmds_fungi_soil3 <- metaMDS(physeq_df, trymax = 200, 
                            autotransform = FALSE, engine = c("monoMDS"))

p3 <-
  PlotSplitNMDS(nmds_fungi_soil3, meta_fungi_fix) +
...
# NOTE! 
# The most correct way is to use avgdist() even if the results won't change much
# it is better to randomize multiple times.
...
# Testing alternative way to visualize distances 
dist_tibble %>% 
  as.matrix() %>%
  as_tibble(rownames = "rows") %>%
  pivot_longer(-rows, names_to="columns", values_to="distances") %>% 
  ggplot(aes(x=rows, y=columns, fill=distances)) +
  geom_tile()
...
dist_tibble %>% 
  ...
  as_tibble(rownames = "rows") %>%
  pivot_longer(-rows, names_to="columns", values_to="distances") %>% 
  mutate(rows = fct_reorder(as.character(rows), rows),
         columns = fct_reorder(as.character(columns), columns)) %>% 
  arrange(desc(distances), .by_group = TRUE) %>% 
  ggplot(aes(x=rows, y=columns, fill=distances)) +
  geom_tile()


Adjadonis2 <- function(dataframe, metadata, formula){
  require(tidyverse)
  require(vegan)
  print(formula)
  ...
  df_meta <-
    metadata %>% 
    filter(Sample_ID %in% rownames(as.matrix(dataframe))) %>% 
    as.data.frame()
...
vegdist(dataframe, method = 'bray') 


used_model <- formula("vegdist(dataframe, method = 'bray') ~ Status + Niche * Treatment")

Adjadonis2(fungi_ev_100itr,
           meta_fungi_fix,
           used_model)


# This function will filter dataframe2 to the same samples present in dataframe1.
# Both dataframe are supposed to have ASV as rows and samples as columns plus
# and ASV_ID column.
MatchSample <-function(dataframe1, dataframe2){
...
# Metadata check fungi
colnames(fungi_table_fix[-1])
rownames(meta_fungi_fix %>% column_to_rownames("Sample_ID"))
identical(colnames(fungi_table_fix[-1]),
          rownames(meta_fungi_fix %>% column_to_rownames("Sample_ID")))
...
# Metadata check
colnames(bact_table_fix)[-1]
rownames(meta_bact_fix %>% column_to_rownames("Sample_ID"))
identical(colnames(bact_table_fix)[-1],
          rownames(meta_bact_fix %>% column_to_rownames("Sample_ID")))

#identical(rownames(bact_metadata), colnames(bact_table_fix)[-1])
#sample_order <- match(colnames(bact_table_fix)[-1], rownames(bact_metadata))
#bact_metadata <- bact_metadata[sample_order, ]
# metadata names are now identical!

# compare fungi and bacteria metadata
meta_bact_fix$Sample_ID
meta_fungi_fix$Sample_ID
dim(meta_bact_fix)
dim(meta_fungi_fix)


# REMOVE SAMPLES ---------------------------------------------------------------
meta_fungi_fix$Status
dim(meta_fungi_fix)
...
fungi_table_fix <-
  fungi_table_fix %>% 
  select(ASV_ID, meta_fungi_fix$Sample_ID)
dim(fungi_table_fix)
...
bact_table_fix <-
  bact_table_fix %>% 
  select(ASV_ID, meta_bact_fix$Sample_ID)
dim(bact_table_fix)
...
PlotAlpha <- function(dataframe, my_labels, y_limit, y_labels){
  rich_plot <-
    dataframe %>% 
    ggplot(aes(x = Treatment, y = mean, fill = Niche), 
           colour="black") +
    geom_bar(stat = "identity", position = position_dodge(), color="black") +
...
Fig_richness <-
  ggarrange(
    alpha_fungi %>%
      ...
      PlotAlpha(rich_sig_post_fungi,  y_limit = 420, y_labels = 420) +
      scale_fill_manual(values = paletteCB6) +
      labs(title = "Fungi Post-Transplant"),
    ...
    alpha_fungi %>%
      ...
      mutate(Niche = as.factor(Niche),
             Niche = fct_relevel(Niche,
                                 "Root", "Rhizosphere", "Soil")) %>%
      PlotAlpha(rich_sig_pre_fungi,  y_limit = 420, y_labels = 420) +
      scale_fill_manual(values = paletteCB6[c(3, 4, 5)]) +
      labs(title = "Fungi Pre-Transplant"), 
    ...
    alpha_bact %>%
      ...
      PlotAlpha(rich_sig_post_bact,  y_limit = 2100, y_labels = 2100) +
      scale_fill_manual(values = paletteCB6) +
      labs(title = "Bacteria Post-Transplant"), 
    ...
    alpha_bact %>%
      ...
      mutate(Niche = as.factor(Niche),
             Niche = fct_relevel(Niche,
                                 "Root", "Rhizosphere", "Soil")) %>%
      PlotAlpha(rich_sig_pre_bact,  y_limit = 2100, y_labels = 2100) +
      scale_fill_manual(values = paletteCB6[c(3, 4, 5)]) +
      labs(title = "Bacteria Pre-Transplant"),
...
split_fungi_pre_rhizo_pval <-
  GetWilcoxPval(split_fungi_pre_rhizo, formula(value ~ Treatment)) 
split_fungi_pre_rhizo[split_fungi_pre_rhizo$Taxonomy %in% split_fungi_pre_rhizo_pval$Taxonomy, ] %>%
  PlotJitt(df = ., Treatment) 


split_fungi_pre_root_pval <-
  GetWilcoxPval(split_fungi_pre_root, formula(value ~ Treatment)) 
split_fungi_pre_root[split_fungi_pre_root$Taxonomy %in% split_fungi_pre_root_pval$Taxonomy, ] %>%
  PlotJitt(df = ., Treatment) 

split_fungi_pre_soil_pval <-
  GetWilcoxPval(split_fungi_pre_soil, formula(value ~ Treatment)) 

split_fungi_pre_soil[split_fungi_pre_soil$Taxonomy %in% split_fungi_pre_soil_pval$Taxonomy, ] %>%
  PlotJitt(df = ., Treatment) 
...
PlotJitt <- function(df, Var) {
  ...
  jitt_plot <- df %>%
    mutate(Log_Abund = -log(Abund + 1 / 20000)) %>%
    ggplot(aes(x = value, y = Taxonomy, color = !!Var, fill = !!Var)) +
    geom_jitter(position = position_jitterdodge(dodge.width = 0.8, jitter.width = 0.5),
                shape = 1) +
    stat_summary(aes(x = Abund), fun.data = mean_sdl, fun.args = list(mult = 1),
                 geom = "pointrange", position = position_dodge(width = 0.8), color = "black",
                 shape = 5, size = 0.15, show.legend = FALSE) +
    scale_x_log10() +
    theme_classic() +
    #geom_vline(xintercept = 100/10530, size=0.5, color="gray") +
    scale_y_discrete(limits = rev)
...
# Extract sequences form tibble and then save to a file
#fungi_DNAseq %>% 
#  mutate(ASV_ID = gsub("ASV", ">ASV", ASV_ID)) %>% 
#  rownames_to_column("Name") %>% 
#  pivot_longer(cols = -Name) %>% 
#  dplyr::select(value) %>% 
#  write.table(x=.,file="otus_fungi.fasta",row.names = FALSE, col.names = FALSE, quote = FALSE)

# Need to remove the trailing 0 form the table to match the taxonomy ASV_IDs.
...
ggplot(test, aes(x=gender, y=number, fill = friend_or_not)) +
...
test1 %>% 
  as.data.frame() %>% 
  slice(1:10) %>% 
  ggplot(., aes(x=Genus, y=value_mean, fill = Treatment)) +
  geom_bar(stat="identity", position = "dodge") +
