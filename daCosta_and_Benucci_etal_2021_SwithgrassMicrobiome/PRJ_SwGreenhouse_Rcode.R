...
# EMAPPING FILES FOR SRA -----------------------------------------------------------------------------
write.csv(sample_data(physeq_fungi_gs_clean), "physeq_fungi_gs_clean.csv")
write.table(sample_data(physeq_bact_gs_clean), "physeq_bact_gs_clean.csv")
...
cmdscale(vegdist(otu_fungi_new, method = "bray"), eig=TRUE) -> pcoa_fungi
as.data.frame(pcoa_fungi$points) -> pcoa_fungi
...
# FIGURE 4 - RF MODELS numeric -----------------------------------------------------------------------------

pca_fungi <-
  PlotNeutralPCA(meta_fungi_filt_2[,1:15], meta_fungi_filt_2) +
  scale_color_manual(values = Pal_soil) +
...
# FIGURE SXX - Random Forest supplementaries ---------------------------------------------------------------
...
# Calculating PCoA in vegan --------------------------------------------------------------------------------
cmdscale(vegdist(otu_bact_new, method = "bray"), eig=TRUE) -> pcoa_bact
as.data.frame(pcoa_bact$points) -> pcoa_bact
...
# FIGURE 5 - RF MODELS numeric -----------------------------------------------------------------------------

pca_bact <-
  PlotNeutralPCA(meta_bact_filt_2[,1:15], meta_bact_filt_2) +
  scale_color_manual(values = Pal_soil) +
  scale_fill_manual(values = Pal_soil) +
  scale_shape_manual(values = c(0,1,2,5,3,8)) +
  theme(legend.position = "none")
...
ggarrange(
  grid.arrange(Fig_4_RF_fungi, top=title3),
  grid.arrange(Fig_5_RF_bact, top=title4),
  as_ggplot(legend1),
  ncol = 1,
  nrow = 3,
  heights = c(1,1,0.1))
...
# Composite Figure for supplementaries 
...
PlotPCOA <- function(df, envfit){
  pcoa <-
    ...
    geom_segment(data = envfit, inherit.aes = FALSE,
                           mapping = aes(x = 0, xend = envfit[,1]/2, y = 0, yend = envfit[,2]/2), 
                           color = "black",
                           arrow = arrow(length = unit(0.02, "npc"))) +
    geom_text(data = envfit,inherit.aes = FALSE,
              mapping = aes(x = 1.05 * envfit[,1]/2, y = 1.05 * envfit[,2]/2, label = Var),
              size = 3) 
return(pcoa)
}
